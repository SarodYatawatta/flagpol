...
class LBFGSB():
    ...
    """
    def __init__(self,x0,lower_bound,upper_bound,max_iter=2,batch_mode=False,history_size=7):
        self.n=x0.size
    ...
    def step(self,cost,gradient):
        ...
        if self.batch_mode and self.n_iter==0:
            self.running_avg=np.zeros_like(self.x)
            self.running_avg_sq=np.zeros_like(self.x)
        ...
        while (self.get_optimality_(self.x,g,self.l,self.u)> self.tol) and (n_iter < self.max_iter):
            ...
            if (line_search_flag):
                if not self.batch_mode:
                    ...
                    alpha=self.backtrack_(cost,gradient,self.x,f,g,xbar-self.x,self.alphabar)
            ...
            batch_changed=self.batch_mode and (n_iter==1 and self.n_iter>1)
            if batch_changed:
                ...
                tmp_grad_1=g_old - self.running_avg # grad-oldmean
                self.running_avg +=tmp_grad_1/self.n_iter
                tmp_grad_2=g_old - self.running_avg # grad-newmean
                self.running_avg_sq += tmp_grad_1 * tmp_grad_2 # +(grad-newmean)(grad-oldmean)
                self.alphabar=1.0/(1.0+self.running_avg_sq.sum()/((self.n_iter-1)*np.linalg.norm(g_old)))
    ...
    def backtrack_(self,cost,gradient,x0,f0,g0,p,alphabar):
        ...
        xk=x0+alphak*p
        f_new=cost(xk)
        prodterm=c1*g0.transpose() @ p
        ci=0
        while (ci<citer and (math.isnan(f_new) or f_new>f0+alphak*prodterm)):
            alphak=0.5*alphak
            xk=x0+alphak*p
            f_new=cost(xk)
            ci=ci+1
